...
# 사이드바 (설정)
# ------------------------------------------------------------------
st.title("📈 SOXL 승률 조건 역탐색 v6")
st.caption("RSI · 추세 · BB · 낙폭 · MACD · ATR · ADX · 장기추세 · 시장레짐 — 9개 조건 조합 백테스트")
...
with st.sidebar:
    ...
    if st.button("🔄 데이터 새로고침", width='stretch'):
        ...
        st.rerun()
    st.divider()
    st.caption("데이터: Yahoo Finance (SOXL / SPY / ^VIX)")
...
# 탭 구성
# ------------------------------------------------------------------
tab_now, tab_match, tab_table, tab_gold, tab_state, tab_gloss = st.tabs([
    "🎯 지금 진입", "📍 패턴 매칭", "📋 탐색 결과", "🏆 황금 분포", "📊 현재 상태", "📖 용어",
])
...
with tab_match:
    ...
    st.caption(f"{cur['rsi_zone']} / {cur['trend']} / {cur['bb_zone']} / {cur['dd_zone']} / "
...
with tab_table:
    ...
    show_cols = [
        "🏆등급", "RSI구간", "추세", "BB위치", "낙폭구간", "MACD방향", "ATR변동성",
        "ADX추세강도", "장기추세", "시장레짐", "최적보유일", "횟수", "승률%",
        "평균수익%", "중앙값수익%", "손실평균%", "Sharpe", "MDD%", "기대값점수", "유의성",
        "평균RSI", "평균MA이격%", "평균BB%B", "평균낙폭%", "평균ATR%", "평균ADX", "유효보유일범위",
    ]
