# -*- coding: utf-8 -*-
"""[v7 반도체판] HTML 렌더링 함수 모음 — 원본 views.py(v6)는 그대로 두고 새로 만든 파일
변경: BB → 거래량 비율, 시장레짐(SPY 200일선 → 반도체 상대강도 + VIX)
...
def sample_details_html(data, close, cond_mask, hold_days=None,
    ...
    for dt in match_dates:
        ...
        samples.append({
            ...
            "거래량비": round(data["vol_ratio"].iloc[i], 2),
    ...
    for s in shown:
        ...
        mr_c = {"안정장세": "#00c853", "반도체약세": "#ffab00"}.get(s["시장레짐"], "#ef5350")
        rows_html += f"""
        <tr style='border-bottom:1px solid #2a2a2a'>
          ...
          <td style='padding:5px 8px;color:#aaa;text-align:right'>{s["거래량비"]}x</td>
    ...
    heads = ["날짜", "진입가", "RSI", "MA이격", "거래량", "낙폭", "ATR", "ADX", "장기추세", "시장레짐"]
    head_html = "".join(
...
def glossary_html():
    ...
    left = "".join([
        ...
        item("거래량 비율 ★v7",
             "<b style='color:#90caf9'>의미:</b> 오늘 거래량 ÷ 최근 20일 평균 거래량.<br>"
             "같은 하락도 거래량이 터졌는지(투매) 조용한지(관심 부족)에 따라 의미가 다름.<br>"
             "<b style='color:#ef5350'>거래량낮음(&lt;0.8배):</b> 관심 적은 움직임.<br>"
             "<b style='color:#ffab00'>거래량보통(0.8~1.5배):</b> 평소 수준.<br>"
             "<b style='color:#66bb6a'>거래량급증(&gt;1.5배):</b> 매도/매수가 몰림."),
        item("RSI 다이버전스",
    ...
    right = "".join([
        ...
        item("시장 레짐 (반도체 상대강도·VIX) ★v7",
             "<b style='color:#90caf9'>의미:</b> SOXX/QQQ 비율의 20일 변화(반도체가 나스닥보다 강한가) + VIX.<br>"
             "<b style='color:#00c853'>안정장세:</b> VIX&lt;30 이고 상대강도 -3% 이상.<br>"
             "<b style='color:#ffab00'>반도체약세:</b> VIX&lt;30 인데 상대강도 -3% 미만 (섹터만 약함).<br>"
             "<b style='color:#ef5350'>위험장세:</b> VIX 30 이상 (시장 전체 공포)."),
        item("유의성 / 기대값",
...
def market_state_html(c):
    ...
    def vc(v): return "#66bb6a" if v < 20 else "#ffab00" if v < 30 else "#ef5350"
    ...
    volc = "#66bb6a" if c["vol_ratio"] >= 1.5 else "#ffab00" if c["vol_ratio"] >= 0.8 else "#ef5350"
    div = c["divergence"]
    divc = "#66bb6a" if div == "상승 다이버전스" else "#ef5350" if div == "하락 다이버전스" else "#aaa"
    ltc = "#66bb6a" if c["long_trend"] == "장기상승" else "#ef5350"
    mrc = {"안정장세": "#00c853", "반도체약세": "#ffab00", "위험장세": "#ef5350"}[c["market_regime"]]
    rsc = "#66bb6a" if c["semi_rs"] >= 3 else "#ef5350" if c["semi_rs"] < -3 else "#ffab00"
    ddc = "#ef5350" if c["dd"] < -7 else "#ffab00" if c["dd"] < -3 else "#fff"
    di_up = c["plus_di"] > c["minus_di"]
    vt = f'{c["vol_today"] / 1e6:.1f}M'
    va = f'{c["vol_avg"] / 1e6:.1f}M'

    return f"""
    <div style='{WRAP}'>
      <div style='font-size:15px;font-weight:bold;color:#90caf9;margin-bottom:14px;border-bottom:1px solid #333;padding-bottom:8px'>
        📊 현재 시장 상태 <span style='font-size:11px;font-weight:normal;color:#555'>※ v7 반도체판 · 마지막 봉 기준(장중이면 진행 중인 값)</span>
      </div>
      <div style='display:flex;flex-wrap:wrap;gap:0 28px'>
        <div style='flex:1 1 300px;min-width:0'>
          ...
          <div style='margin-bottom:12px'><div style='color:#90caf9;font-size:12px;font-weight:bold;margin-bottom:4px'>거래량 비율 ★v7</div>
            <div style='font-size:13px;padding:2px 8px'>오늘 <b>{vt}</b> / 20일평균 <b>{va}</b><br>
              비율 <b style='color:{volc}'>{round(c["vol_ratio"],2)}배</b> → <b style='color:{volc}'>{c["vol_zone"]}</b>
              <span style='font-size:11px;color:#555'>(낮음&lt;0.8 / 보통0.8~1.5 / 급증&gt;1.5)</span></div></div>
        </div>
        <div style='flex:1 1 300px;min-width:0'>
          ...
          <div style='border-top:1px solid #333;padding-top:10px'><div style='color:#90caf9;font-size:12px;font-weight:bold;margin-bottom:4px'>🌐 시장 레짐 (반도체 상대강도 + VIX) ★v7</div>
            <div style='font-size:13px;padding:2px 8px'>반도체 상대강도(SOXX÷QQQ, 20일) <b style='color:{rsc}'>{c["semi_rs"]:+.2f}%</b><br>
              VIX <b style='color:{vc(c["vix"])}'>{round(c["vix"],2)}</b> ({c["vix_zone"]})<br>
              종합 <b style='color:{mrc}'>{c["market_regime"]}</b>
              <span style='font-size:11px;color:#555'>(위험=VIX≥30 · 반도체약세=상대강도&lt;-3% · 안정=그 외)</span></div></div>
        </div>
      </div>
    </div>"""
...
def gold_distribution_html(df_gold):
    ...
    cols = [("RSI구간", "RSI구간"), ("추세", "추세"), ("거래량", "거래량"), ("낙폭구간", "낙폭구간"),
